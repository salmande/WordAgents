...
if( isset($_POST['updateSettings']) ){
	...
	if( is_admin() )
	{
		...
		$no_of_days_for_new_orders = isset($_POST['no_of_days_for_new_orders']) ? $_POST['no_of_days_for_new_orders'] : 0;
		wad_update_option('no_of_days_for_new_orders',$no_of_days_for_new_orders);
		
		$no_of_testing_orders = isset($_POST['no_of_testing_orders']) ? $_POST['no_of_testing_orders'] : '';
		wad_update_option('no_of_testing_orders',$no_of_testing_orders);
		...
		if( $spp_id == '3653' )
		{
			$test_orders_only = isset($_POST['test_orders_only']) ? $_POST['test_orders_only'] : 'no';
			wad_update_option('test_orders_only',$test_orders_only);
		}
	}
}
...
if( is_admin() ){
	...
	$no_of_testing_orders = wad_get_option('no_of_testing_orders');
	$test_orders_only = wad_get_option('test_orders_only');
}
if( is_admin() || is_assigner() ){
		...
		<div class="container">
			...
			<form method="post">
				...
				<div class="form-group">
					...
					<input type="number" min="0" name="no_of_days_for_new_orders" class="form-control" placeholder="4" value="<?php echo $no_of_days_for_new_orders; ?>" />
				</div>
				<div class="form-group">
					<label>No. of Testing Orders</label>
					<input type="number" min="0" name="no_of_testing_orders" class="form-control" placeholder="4" value="<?php echo $no_of_testing_orders; ?>" />
				</div>
				<?php endif; ?>
				...
				<div class="form-group">
					<div class="radio-inline">
						<label class="mr-4">test_orders_only</label>
						<label class="radio">
							<input type="radio" <?php if( isset($test_orders_only) && $test_orders_only == 'yes') { echo 'checked'; } ?> name="test_orders_only" value="yes" />
							<span></span>
							Yes
						</label>
						<label class="radio">
							<input type="radio" <?php if( isset($test_orders_only) && $test_orders_only == 'no') { echo 'checked'; } ?> name="test_orders_only" value="no" />
							<span></span>
							No
						</label>
					</div>
					<span class="text-muted">It is for our test users only</span>
				</div>
				<?php endif; ?>
