...
{
	if( $current_url == 'orders'
		|| $current_url == 'orders/working'
		|| $current_url == 'orders/editing'
		|| $current_url == 'orders/revisions/editor'
		|| $current_url == 'orders/revisions/client'
		|| $current_url == 'orders/complete'
		|| $current_url == 'orders/all' 
		|| $current_url == 'accounting/pending'
		|| $current_url == 'accounting/all'
		|| $current_url == 'order/single'
	){
		...
		if( $current_url == 'orders/working' ) // Working
		{ 
		...
		else if( $current_url == 'accounting/pending') // Pending Earnings
		{
		...
		else if( $current_url == 'orders/editing') // Editing
		{
		...
		else if( $current_url == 'orders/revisions/editor') // Editor Revisions
		{	
		...
		else if( $current_url == 'orders/revisions/client') // Client Revisions
		{	
		...
		else if( $current_url == 'orders/complete') // Complete
		{
		...
		else if( $current_url == 'accounting/all') // All Earnings - Accounting
		{
		...
		else if( $current_url == 'orders/all') // All Orders
		{	
			/* $assigned_orders_array = wad_explode_assigned_orders($current_user['assigned_orders']);
		...
		{/* 
			...
			$globals['new_orders_count'] = $_SESSION['new_orders_count'] = $_SESSION['new_orders_count_with_date_due'] = $total; */
			
		}
		
		if( wad_test() ){
			if( $current_url == 'order/single'){
				$per_page = -1;
		...
		if( isset($orders_all) && !empty($orders_all) ){
			...
			foreach($orders_all as $order){
				...
				if( $current_url == 'orders/complete' || $current_url == 'accounting/pending' || $current_url == 'accounting/all'){
...
else
{
	...
	{	// All Users Count
		if(is_assigner() ){
			$all_users_count = wad_get_total_count("users", "role='Writer' OR role='Editor'");
		}else{
			$all_users_count = wad_get_total_count("users");
		}
	}
	else if( $current_url == 'users/writers' )
	{	// All Writers Count
		$all_writers_count = wad_get_total_count("users", "role='Writer'");
	}
	else if( $current_url == 'users/assigners' )
	{	// All Assigners Count
		$all_assigners_count = wad_get_total_count("users", "role='Assigner'");
	}
	else if( $current_url == 'users/admins' )
	{	// All Admins Count
		$all_admins_count = wad_get_total_count("users", "role='Admin'");
	}
	else if( $current_url == 'users/editors' )
	{	// All Editors Count
		$all_editors_count = wad_get_total_count("users", "role='Editor'");
	}
	else if( $current_url == 'users/edit')
	{
		$url = $_REQUEST['url']; // "users/edit/123" where 123 is user's SPP ID.
		$user_spp_id = basename(parse_url($url, PHP_URL_PATH)); //123
		...
		$all_orders_result = wad_select_query( $query_from,$query_select,$query_where,$query_order);
		$all_orders_total = wad_get_total_count( $query_from,$query_where);
		$all_orders_records = mysqli_fetch_all($all_orders_result, MYSQLI_ASSOC);
		
		//Working Orders
		$status = "(status='5')"; //Working
		$query_where = "order_id IN ({$order_ids_IN}) AND {$status}";
		$query_order = "ORDER BY assigned_end ASC";
		
		$working_orders_result = wad_select_query( $query_from,$query_select,$query_where,$query_order);
		$working_orders_total = wad_get_total_count( $query_from,$query_where);
		$working_orders_records = mysqli_fetch_all($working_orders_result, MYSQLI_ASSOC);
		...
		$editing_orders_result = wad_select_query( $query_from,$query_select,$query_where,$query_order);
		$editing_orders_total = wad_get_total_count( $query_from,$query_where);
		$editing_orders_records = mysqli_fetch_all($editing_orders_result, MYSQLI_ASSOC);
		
		//Editor Revision Orders
		$status = "(status='6')"; //Editor Revision
		$query_where = "order_id IN ({$order_ids_IN}) AND {$status}";
		$query_order = "ORDER BY started ASC";
		
		$revision_orders_result = wad_select_query( $query_from,$query_select,$query_where,$query_order);
		$revision_orders_records = mysqli_fetch_all($revision_orders_result, MYSQLI_ASSOC);
		$editor_revision_orders_total = wad_get_total_count( $query_from,$query_where);

		//Client Revision Orders
		$status = "(status='9')"; //Client Revision
		$query_where = "order_id IN ({$order_ids_IN}) AND {$status}";
		$query_order = "ORDER BY started ASC";
		
		$revision_orders_result = wad_select_query( $query_from,$query_select,$query_where,$query_order);
		$revision_orders_records = mysqli_fetch_all($revision_orders_result, MYSQLI_ASSOC);
		$revision_orders_total = wad_get_total_count( $query_from,$query_where);
		
		//Completed Orders
		$query_where = "order_id IN ({$order_ids_IN}) AND status=3";
		$query_order = "ORDER BY started ASC";
		
		$completed_orders_result = wad_select_query( $query_from,$query_select,$query_where,$query_order);
		$completed_orders_total = wad_get_total_count( $query_from,$query_where);
		$completed_orders_records = mysqli_fetch_all($completed_orders_result, MYSQLI_ASSOC);
	}
}