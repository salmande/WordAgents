				...
				<div class="subheader py-2 py-lg-4 subheader-transparent" id="kt_subheader">
					<div class="container align-items-center justify-content-between flex-wrap flex-sm-nowrap">
						...
						<div class="d-flex flex-wrap mr-2">
								...
								<div class="mr-2 mb-2 mt-2">
									<?php $writers = wad_get_users("spp_id, name","role='writer' AND is_archive=0"); ?>
									<select name="writers[]" class="form-control dropdown-writers selectpicker" data-none-selected-text="Select writer(s)" data-actions-box="true" data-live-search="true" data-count-selected-text="{0} writers selected" data-selected-text-format="count" data-width="300px" multiple="multiple">
								...
								<div class="mr-2 mb-2 mt-2">
									<?php $writers_archived = wad_get_users("spp_id, name","role='writer' AND is_archive=1"); ?>
									<select name="writers_archived[]" class="form-control dropdown-writers selectpicker" data-none-selected-text="Add archived writer(s)" data-actions-box="true" data-live-search="true" data-count-selected-text="{0} archived writers selected" data-selected-text-format="count" data-width="220px" multiple="multiple">
										<?php foreach($writers_archived as $writer): ?>
											<option value="<?php echo $writer['spp_id']; ?>"><?php echo $writer['name']; ?></option>
										<?php endforeach; ?>
									</select>
								</div>
								<div class="mr-2 mb-2 mt-2">
									...
									$columns = array(
										...
										'rejected_orders_count' => 'Total Number of Orders Rejected',
										'rejected_orders' => 'Orders Rejected',
										
										'order_turnaround' => 'Order Turnaround time',
										
										'late_orders_count' => 'Total Number of Late Orders',
										'late_orders' => 'Late Orders',
									);
									
									?>
									<select name="columns[]" class="form-control selectpicker" data-live-search="true" data-none-selected-text="Enable/disable columns" data-actions-box="true"  data-count-selected-text="{0} columns selected" data-selected-text-format="count" data-width="410px" multiple="multiple">
										<?php foreach($columns as $key=>$val): ?>
											<option value="<?php echo $key; ?>" <?php if(in_array($key,$globals['stats_writers_columns_default'])){ echo 'selected'; } ?>><?php echo $val; ?></option>
										<?php endforeach; ?>
									</select>
								</div>
								<div class="mb-2 mt-2">
					...
					<div class="card card-custom">
						...
						<div class="card-header flex-wrap border-0 pt-6 pb-6">
							<div class="card-title">
								<h3 class="card-label">Writers</h3>
							</div>
						...
						<div class="card-body">
							<!--begin: table-->
							<?php echo wad_stats_writers($stats); ?>
							<!--end: table-->
						</div>
						<!--end::Body-->
					</div>
