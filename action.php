...
if( $action == 'editor_submit_editing_order' )  // editor_submit_editing_order - START
{
	...
	$order_doc_link = $_POST['doc_link'];
	
	$assigned_writers = $_POST['assigned_writers'];
	$assigned_editors = $_POST['assigned_editors'];
	...
	$post = array_merge($post, $_POST['order_client_info']);
	$order_client_email = $post['order_client_email'];
	...
	if( wad_get_option('send_emails') == 'yes' )
	{
		...
		$p2 = "<p>This article is all set for your review! You can find the completed document here: <a href='{{doc_link}}'>{{doc_link}}</a></p>";
