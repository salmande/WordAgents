 ...
 * @param string $action - orders page
 * @param boolean $showPrevNext - whether to show previous and next page links
 * @return $pagination html
 */
function wad_pagination($currentPage, $itemCount, $itemsPerPage, $adjacentCount, $pageLinkTemplate, $action, $showPrevNext = true) {
				...
				if ($lastPage != 1) {
					...
					if ($showPrevNext) {
						if ($currentPage == $firstPage) {
							...
							$output .= '<a href="' . (is_callable($pageLinkTemplate) ? $pageLinkTemplate($currentPage - 1) : sprintf($pageLinkTemplate, $currentPage - 1, $itemsPerPage)) . '" class="page btn btn-icon btn-xs btn-light mr-2 my-1"><i class="ki ki-bold-arrow-back icon-xs"></i></a>';
						}
					}
					if ($firstAdjacentPage > $firstPage) {
						$output .= '<a href="' . (is_callable($pageLinkTemplate) ? $pageLinkTemplate($firstPage) : sprintf($pageLinkTemplate, $firstPage, $itemsPerPage)) . '" class="page btn btn-icon btn-xs border-0 mr-2 my-1 btn-light">' . $firstPage . '</a>';
					...
					for ($i = $firstAdjacentPage; $i <= $lastAdjacentPage; $i++) {
						if ($currentPage == $i) {
							...
							$output .= '<a href="' . (is_callable($pageLinkTemplate) ? $pageLinkTemplate($i) : sprintf($pageLinkTemplate, $i, $itemsPerPage)) . '" class="page btn btn-icon btn-xs border-0 mr-2 my-1 btn-light">' . $i . '</a>';
						}
					}
					if ($lastAdjacentPage < $lastPage) {
						...
						$output .= '<a href="' . (is_callable($pageLinkTemplate) ? $pageLinkTemplate($lastPage) : sprintf($pageLinkTemplate, $lastPage, $itemsPerPage)) . '" class="page btn btn-icon btn-xs border-0 mr-2 my-1 btn-light">' . $lastPage . '</a>';
					}
					if ($showPrevNext) {
						if ($currentPage == $lastPage) {
							...
							$output .= '<a href="' . (is_callable($pageLinkTemplate) ? $pageLinkTemplate($currentPage + 1) : sprintf($pageLinkTemplate, $currentPage + 1, $itemsPerPage)) . '" class="page btn btn-icon btn-xs btn-light mr-2 my-1"><i class="ki ki-bold-arrow-next icon-xs"></i></a>';
						}
					}	
				}
				...
				<form class="pagination-form">
					<select class="form-control form-control-xs font-weight-bold mr-4 border-0 bg-light select-per-page" style="width: 75px;" name="per_page">
						<?php foreach($orders_per_page_dropdown_array as $dropdown_item): ?>
							<option value="<?php echo $dropdown_item; ?>" <?php if($itemsPerPage==$dropdown_item){ echo 'selected';} ?>><?php echo $dropdown_item; ?></option>
						<?php endforeach; ?>
					</select>
...
function wad_get_current_url($domain_exclude = null){
	...
	$wad_url = str_replace('wad-orders','orders',$wad_url);
	$wad_url = str_replace('wad-admin','admin',$wad_url);
	
	if( $domain_exclude )
		return $wad_url;
	
	return BASE_URL.'/'.$wad_url;
}
