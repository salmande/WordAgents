...
function wad_stats_writers($stats = 'stats'){
	...
	if( $ajax )
		...
		$columns = isset($_REQUEST['columns']) ? $_REQUEST['columns'] : $globals['stats_writers_columns_default'];
	
	$writers = isset($_REQUEST['writers']) ? $_REQUEST['writers'] : array();
	$writers_archived = isset($_REQUEST['writers_archived']) ? $_REQUEST['writers_archived'] : array();
	if( $export=='all'){
		$writers = array();
		// $writers_archived=array();
		$page=1;
		$per_page='All';
		// $per_page=4;
	}
	...
	if(!empty($writers))
	$writers_ids_IN = wad_set_get_IN($writers);

	if(!empty($writers_archived))
	$writers_archived_ids_IN = wad_set_get_IN($writers_archived);
	...
	if( isset($writers_ids_IN) && isset($writers_archived_ids_IN) ){
		$query_where .= " AND spp_id IN ({$writers_ids_IN},{$writers_archived_ids_IN})"; //selected both archived and non-archived writers
	}else if(isset($writers_ids_IN)){
		$query_where .= " AND spp_id IN ({$writers_ids_IN})"; //selected non-archived writers
	}else if(isset($writers_archived_ids_IN)){
		$query_where .= " AND ( spp_id IN ({$writers_archived_ids_IN}) OR is_archive=0 )"; //selected archived + all of non-archived writers.
	}else{
		$query_where .= " AND is_archive=0"; //all of non-archived writers
	}
	...
	$pagination='';
	if( $export!='all')
	$pagination = wad_pagination($page, $stats_writers_total, $per_page);
	
	$is_writers_archived_ids_IN = isset($writers_archived_ids_IN) ? true : false;
	...
	<div class="overflow-auto"><table class="table table-stats table-<?php echo $stats; ?>">
		...
		<?php endif; ?>
			...
			<tr>
				...
				<?php if($is_writers_archived_ids_IN): ?>
			...
			<tr>
				<td>
					<div class="name-column">
						...
						<?php else: ?>
							<span class="text-dark-75 font-weight-bolder font-size-lg mb-0 text-name"><a class="text-dark-75 text-hover-primary" href="<?php echo BASE_URL; ?>/users/edit/<?php echo $writer_spp_id; ?>"><?php echo $writer_name; ?></a></span><br>
							<a href="<?php echo BASE_URL; ?>/users/edit/<?php echo $writer_spp_id; ?>" class="text-muted font-weight-bold text-hover-primary text-email"><?php echo $writer_email; ?></a>
							<div>
								<?php if( $writer['is_archive'] ): ?>
									<span class="text-danger text-archived">Archived Account</span>
								<?php else: ?>
									<span class="text-signin"><a href="<?php echo BASE_URL; ?>?action=sign_in_as_user_using_admin&user=<?php echo $writer_spp_id; ?>">Sign in as user</a></span>
								<?php endif; ?>
							</div>
						<?php endif; ?>
					</div>
				...
				<?php if($is_writers_archived_ids_IN): ?><td class="hide"><?php echo ( $writer['is_archive'] ) ? 'Yes' : 'No'; ?></td><?php endif; ?>
			...
			<tr class="row-total bg-primary-o-25">
				...
				<td class="hide"></td>
				<?php if($is_writers_archived_ids_IN): ?><td class="hide"></td><?php endif; ?>
	...
	if( $export=='all'){
		...
		$writers_stats_file = BASE_DIR.'/stats-generated/'.$filename.'.xls';
		$writers_stats->save($writers_stats_file); 
		
		echo BASE_URL . '/stats-generated/'.$writers_stats_filename;	
		die();
	}
	return $html;
